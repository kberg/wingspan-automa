...
class Automa:
  ...
  def playCard(self, game, goal):
    ...
    if action == 'g':
      if goal == 'high':
        result = self.highone(game.display)
      elif goal == 'low':
        result = self.lowone(game.display)
      else:
        raise "Yo"
  ...
  def score(self, parameters):
    ...
    scores = {
        'eggs': self.eggs,
        'birds': self.birds * parameters['ppb'],
...
def run_simulation(params, writer):
  ...
  fmtdparams = '{c[ppb]}-{c[autombon]}-{c[deck]}-{c[goal]}'.format(c = params)
...
def run_all():
  ...
  csv_columns = ['parameters', 'p_ppb', 'p_autombon', 'p_deck', 'p_goal', 'eggs', 'birds', 'game-end-bonus', 'round-end-bonus', 'total']
  try:
    with open('scores.csv', 'w') as csvfile:
      ...
      for ppb in ppbs:
        for autombon in autombons:
          for deck in decks:
            for goal in goals:
              params = { 'ppb': ppb, 'autombon': autombon, 'deck': deck, 'goal': goal}
